...
Additionally, charms supporting basic authentication, [such as Traefik](https://documentation.ubuntu.com/traefik-k8s-charm/latest/how-to/enable-basicauth/), accept a `<username>:<hashed-password>` config option by which the user can configure basic authentication. The supported hashing algorithm are MD5, SHA1, or `bcrypt`, [as per official documentation](https://doc.traefik.io/traefik/middlewares/http/basicauth). Also following the official guidelines, we recommend cloud admins to use `htpasswd` for hashing the password and formatting the configuration string.
