...
To limit variance in COS deployments, the COS Terraform module offers per-component `revision` and `resources` configuration. See the [available COS Terraform configuration](../../../terraform/cos/variables.tf) for details. Pinning the values of these fields ensures that the components remain unchanged, unless it was explicitly targeted for upgrades.
