...
```{mermaid}
graph BT
    subgraph JujuModel["🔷 Juju Model"]
        Ubuntu["📦 Ubuntu Charm<br/>(Application source)"]
        OtelCol-Integrator["⚙️ OpenTelemetry Collector Integrator"]
        OtelCol["🔄 OpenTelemetry Collector"]
    end

    Rsyslog["📋 External rsyslog server<br/>(TLS on port 6514)"]


Ubuntu ---|"juju-info"| OtelCol
OtelCol-Integrator -->|"external-config"| OtelCol
OtelCol -->|"syslog/rfc5424<br/>over TLS"| Rsyslog
```
