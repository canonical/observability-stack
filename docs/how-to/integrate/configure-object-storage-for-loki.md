...
myst:
  html_meta:
    description: "Connect S3-compatible object storage to a Loki on Juju deployment through s3-integrator, with production planning notes and Minio-based testing links."
---

# How to connect object storage to Loki on Juju

Loki on Juju needs an S3-compatible object store for durable storage. The
...
Use this guide to connect an existing object store to a Loki on Juju
deployment.
...
- A Loki on Juju deployment (at minimum the `loki-coordinator-k8s`
  application). If you do not have one yet, follow
  [How to deploy Loki on Juju](/how-to/deploy-and-manage/deploy-loki-on-juju) first.
...
Do this **before** wiring anything into Loki; changing the answer to any of
these later is disruptive.

- Use a dedicated bucket. Keep Loki in its own bucket unless you have a
  clear reason to share one.
- Use dedicated credentials for that bucket, scoped so they can only
  access what Loki needs.
...
- Keep the S3 endpoint stable before you start wiring producers to the
  `logging` endpoint; changing it later forces a reconfiguration of Loki
  workers.
...
secret to the `loki-s3` application:
...
juju grant-secret secret:<secret-ID> loki-s3
```
...
```bash
juju config loki-s3 tls-ca-chain="$(base64 -w0 ca-chain.pem)"
```

## 4. Integrate Loki with the S3 integrator
...
joining Loki workers to the coordinator. That keeps the storage
...
You should see an `s3` integration between `loki` and `loki-s3`, and both
