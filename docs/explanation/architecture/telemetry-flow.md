...
The Opentelemetry Collector machine charm aims to collect both host and application telemetry in deployment scenarios where a full virtual machine is being used. It is powered by the `opentelemetry-collector` snap, and handed a configuration containing all scrape targets available on the machine, including a `node_exporter` endpoint provided by the snap itself.
