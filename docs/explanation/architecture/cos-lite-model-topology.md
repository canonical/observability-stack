...
OpenTelemetry Collector is the single fan-in point for telemetry generated by COS Lite components: it scrapes every component's metrics endpoint and remote-writes them into Prometheus, and it receives every component's logs and forwards them to Loki. Routing self-monitoring through the collector rather than relating each component straight to a backend means processing, relabeling and topology injection apply uniformly, and it matches how COS is wired.

Components in COS Lite send their dashboards to OpenTelemetry Collector. OpenTelemetry Collector then forwards all the collected dashboards to Grafana.
