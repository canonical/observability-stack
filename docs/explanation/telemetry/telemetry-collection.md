...
The standard practice is for a workload to expose its metrics so they can be scraped; this is a **pull** mechanism, because the _observer_ (Prometheus, OpenTelemetry Collector, etc.) fetches — _pulls_ — the metrics from the _observed workload_.
