...
- Traces -> Logs: include the trace id/span id in application logs so a log viewer (e.g. Loki) can link to the trace in Tempo.
- Traces -> Metrics: attach trace information as Prometheus exemplars to time series samples so Mimir/Prometheus can link metrics to traces.
...
- Verify traces: use Tempo (e.g. Grafana Tempo UI) to confirm traces arrive and contain expected trace ids.
- Verify logs: check a log entry in Loki; confirm it contains `trace_id`/`traceparent` fields and that the UI links to Tempo.
