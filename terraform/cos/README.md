...
Grafana can correlate metrics, logs, and traces signals, allowing you to navigate between related signals to make troubleshooting issues easier.

In order to enable telemetry correlation in COS across logs, metrics, and traces, create a `main.tf` file with the following content.

```hcl
# COS module that deploy the Canonical Observability Stack with telemetry correlation enabled
module "cos" {
    source                        = "git::https://github.com/canonical/observability-stack//terraform/cos"
    model                         = "cos"
    channel                       = "2/edge"
    ...
    mimir_bucket                  = "mimir"
    tempo_bucket                  = "tempo"
    # to configure telemetry correlation
...
This will deploy COS with the required relations and config to set up Mimir, Loki, and Tempo data sources for telemetry correlation. To use this feature, the charmed workload sending telemetry to COS must be instrumented in a way so that Grafana can link metrics, logs, and traces.

<!-- TODO: add the rest of the doc links once published -->
> See more: [traces-to-logs](https://discourse.charmhub.io/t/tempo-ha-docs-how-to-enable-correlation-from-traces-to-logs/19029),
> [traces-to-metrics](https://discourse.charmhub.io/t/topic/19094)

> **Note:** Deploying COS with `telemetry_correlation=true` will configure `mimir` to enable exemplar storage.