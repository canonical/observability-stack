...
module "alertmanager" {
  source         = "git::https://github.com/canonical/alertmanager-k8s-operator//terraform?ref=feat/charm-input-map"
  model          = var.model
  config_options = var.charms["alertmanager"]
}

module "catalogue" {
  source         = "git::https://github.com/canonical/catalogue-k8s-operator//terraform?ref=feat/charm-input-map"
  model          = var.model
  config_options = var.charms["catalogue"]
}
