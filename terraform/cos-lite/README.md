...
| <a name="input_model"></a> [model](#input\_model) | Model configuration. When model.uuid is set, looks up the existing model. When null (and model\_uuid is also null), creates a new model with the given name/cloud/config. | <pre>object({<br/>    uuid = optional(string)<br/>    name = optional(string, "cos-lite")<br/>    cloud = optional(object({<br/>      name   = string<br/>      region = optional(string)<br/>    }))<br/>    config         = optional(map(string), {})<br/>    create_timeout = optional(string, "30m")<br/>  })</pre> | `{}` | no |
| <a name="input_model_uuid"></a> [model\_uuid](#input\_model\_uuid) | DEPRECATED: Use var.model instead. UUID of an existing model to deploy to. Mutually exclusive with var.model. | `string` | `null` | no |
